...
interface Props {
  ...
  onButtonClick: (postCodeSearch: string) => void;
...
class AddressSelectorSimple extends Component<Props, State> {
  constructor(props) {
    super(props);
  ...
  handleSearch = (e, postCodeSearch) => {
    e.preventDefault();
    this.props.onButtonClick(postCodeSearch);
    return false;
  };
    ...
    if (!showAddressFields && !selectedAddress) {
      return (
        <AddressRow flexWrap="wrap" style={{ postion: 'relative' }} id={id}>
          ...
          <Box width={[1, 1, 1 / 3, 1 / 2]} pr={[0, 0, 2, 2]} mb={[2, 2, 0, 0]}>
            <TextInput
              ...
              onKeyPress={e => {
                if (e.key === 'Enter' && !searching && postCodeSearch) {
                  this.handleSearch(e, postCodeSearch);
                }
              }}
              disabled={disabled}
            />
          </Box>
          <Box width={[1, 1, 1 / 3, 1 / 2]}>
            <Button
              id={searchButtonId}
              full
              solid
              ...
              loading={searching}
              onClick={e => this.handleSearch(e, postCodeSearch)}
            >
              Find Address
            </Button>
          </Box>
...
(AddressSelectorSimple as any).propTypes = {
...
(AddressSelectorSimple as any).defaultProps = {
  buildingNumber: '',
  postCode: '',
  city: '',
  street: '',
...
export default AddressSelectorSimple;
