...
const StepProgressBar: React.SFC<Props> = props => {
  const { steps, progress, currentStep } = props;

  return (
    <Wrapper {...props}>
      {steps.map((step, index) => {
        const iconName = currentStep === index + 1 || index + 1 > currentStep ? step.icon : 'check';
...
(StepProgressBar as any).propTypes = {
  items: PropTypes.array,
  /** A Value between 0 and 100 */
  progress: PropTypes.number
};

(StepProgressBar as any).defaultProps = {
  currentStep: 1,
  steps: [
